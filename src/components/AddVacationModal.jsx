...
function AddVacationModal({ open, onClose, dentist, vacations = [], onSubmit, onDeleteVacation }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/25 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="p-5 overflow-y-auto flex-1 min-h-0 space-y-4">
          {vacations.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-slate-600 mb-2">Текущи отпуски</h4>
              <ul className="space-y-2">
                {vacations.map((v) => (
                  <li
                    key={v.id}
                    className="flex items-center justify-between gap-2 py-2 px-3 rounded-lg bg-slate-100 border border-slate-200"
                  >
                    <span className="text-sm text-slate-800">
                      {formatDate(v.start_date)} – {formatDate(v.end_date)}
                      {v.note && (
                        <span className="block text-xs text-slate-500 mt-0.5">{v.note}</span>
                      )}
                    </span>
                    <button
                      type="button"
                      onClick={() => onDeleteVacation?.(v.id)}
                      className="p-1.5 rounded text-slate-500 hover:bg-red-900/60 hover:text-red-400 shrink-0"
                      title="Изтрий отпуск"
          ...
          <h4 className="text-sm font-medium text-slate-600 mb-2">Добави нов отпуск</h4>
