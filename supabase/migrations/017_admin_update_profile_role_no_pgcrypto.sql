-- RPC за промяна на роля чрез админ парола (без pgcrypto)
-- Запазва admin_pin в clinic_settings и прави директно сравнение

insert into public.clinic_settings (key, value)
values ('admin_pin', '1918138')
...
begin
  ...
  select value into stored_pin from clinic_settings where key = 'admin_pin' limit 1;
  ...
  if trim(admin_pin) != stored_pin then
